...
export default function RegexTester() {
  ...
  return (
    <div className="space-y-4">
      ...
      {matches.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-gray-700 mb-2">Matches ({matches.length})</h3>
          <div className="flex flex-wrap gap-2">
            {matches.map((m, i) => (
              <span key={i} className="px-2 py-1 bg-green-100 text-green-800 rounded text-sm font-mono">{m}</span>
            ))}
          </div>
        </div>
      )}
